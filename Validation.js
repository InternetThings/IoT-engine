...
ValidateUserInformation = function(newUsername, newUserEmail, passwordFirst, passwordSecond) {
    ...
    if (newUserEmail !== null && newUserEmail !== undefined && newUserEmail !== '') {}
    else {
        valid = false;
        throw new Error("No email was entered.");
    }

    if (passwordFirst !== null && passwordFirst !== undefined && passwordFirst !== '') {}
    else {
        valid = false;
        throw new Error("First password was not entered.");
    }

    if (passwordSecond !== null && passwordSecond !== undefined && passwordSecond !== '') {}
    else {
        valid = false;
...
//Validate a new condition
ValidateCondition = function(condition) {
    var valid = true;
    if (condition.accessToken_id !== null && condition.accessToken_id !== undefined && condition.accessToken_id !== '') {}
    else {
        valid = false;
        throw new Error("No sensor chosen.");
    }

    if (condition.operator !== null && condition.operator !== undefined && condition.operator !== '') {}
    else {
        valid = false;
...
//Evaluate data and a condition, return true or false wether the data meets the condition's requirement.
